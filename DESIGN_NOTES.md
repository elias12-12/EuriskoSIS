# Design notes — the working record

**The deliverable is [DESIGN.md](DESIGN.md).** This is the raw material it was
distilled from: notes written phase by phase, at the time each decision was
made, per PROJECT_PLAN.md Phase 7 — "write this as you go, not at the end;
you'll lose the actual reasoning behind chunking/caching decisions otherwise."

Kept rather than deleted because it is the only place the *rejected* options
survive, and because a decision is easier to revisit when you can still see what
it was weighed against. Each phase's section also ends with a "still open" list,
which is where the honest account of what is unfinished lives.

Read DESIGN.md for the conclusions. Read this for why they are the conclusions.
...
The pair that matters is Karim and Rania: same course, **different reasons**. The
test asserts the reason, not just the boolean, because a refusal for the wrong
reason would still score as a pass on the plan's literal exit check.

> **Corrected once the database was actually running.** This section originally
> claimed Rania is refused by the probation cap "not the prerequisite — she has
> passed MECH 210", and that Karim would be told yes. Both were wrong, and both
> were written from assumption rather than from the data.
>
> Rania has never taken MECH 210 (she is a 2nd-year repeating PHYS 101 and
> MATH 102), so she is blocked *twice over*: prerequisite and cap. The cap is
> still what makes her the probation case, because it is the **independent**
> reason — it would refuse her even if the prerequisite were met — and that is
> what the test now asserts.
>
> Karim is the one student who has passed MECH 210 (B, SP2026), and he is
> **already registered** for MECH 310 this term, so the correct answer to "am I
> allowed to register" is "you already are". Which means MECH 310 cannot produce
> a `yes` from anyone, and a test comparing booleans across Karim and Rania
> proves nothing. `verify_phase5.py` now compares the *blocking rules*, and adds
> ENGR 450 — which Karim can take and Rania cannot — so that "the tool can say
> yes at all" is asserted somewhere.
>
> Recorded rather than quietly edited: guessing at fixture data and writing the
> guess into a design document is exactly the failure mode this project keeps
> warning about, and it survived three phases because nothing had run.
...
### The degree-progress panel is the whole point of this phase

The plan names "surplus credits in one category don't offset another" as the #1
thing students in the brief get wrong, and Jad (S2023027) is the dataset's proof:
55 of 72 credits reads as 76% complete while he holds 1 of 4 General Education
courses and has never started the 8-credit capstone category.

So `DegreeProgressPanel.tsx` is built to make that mistake hard rather than to
...
- **The credit total is never a single progress bar.** One bar across 72 credits
  is precisely the picture that lies. It appears as a number, captioned that it
  is *not* the graduation test.
- **Surplus credits are drawn outside the category bar**, in a detached chip on
  the far side of a dashed rule, captioned with where they cannot go. Rendered
  inside the bar they would say the opposite of the rule.
- **An untouched category gets the loudest treatment on the page** — it is what
  a percentage hides and what the student most needs to see.

The three category states differ in border weight and badge *text*, not only in
colour, so they survive a reader who cannot distinguish the colours.

The portal also refuses to undo the backend's care about a null GPA: Lynn's
shows as `—` with "no graded courses yet", never `0.00`, which would read as
failing.

### Two principals, and a test that they stay separate

`admin_sessions` is a separate table from `student_sessions`, not a nullable
`student_id` on a shared one. The two are different kinds of principal with
different powers, and a single table with a nullable owner column is exactly the
shape where a forgotten `WHERE student_id IS NOT NULL` turns an admin token into
a student one — or the reverse.

`verify_phase6.py` asserts both directions: a student token is refused by
`/admin/*`, an admin token is refused by `/me/*`. That test would fail on the
merged design, which is why it is worth writing even though nothing currently
merges them.

The frontend mirrors this: `api.ts` keeps two tokens under separate keys, so
signing into the admin panel does not sign you into a student portal.

### The exit check is a claim about where a read happens

"Changing a behaviour setting changes the very next chat response, with no
restart" is not a property of the settings form. It is a property of
`assistant_config.load` running at the start of every turn and being
deliberately uncached — decided back in Phase 4 for exactly this reason.

`verify_phase6.py` measures it the only way that is measurable: set
`response_length` to brief, ask a question, set it to detailed, ask the same
question, compare lengths. Tone is not tested, because "did this get friendlier"
has no assertion.

### The browsers are read-only, and the admin panel says so

There are no write endpoints behind them. The dataset is frozen ("Do not edit.
Any change invalidates comparison across teams"), and an admin panel that could
edit a transcript would also be a way to make the five test students disagree
with every verification script in `backend/scripts/`.

`browse.py` is a separate module from `records.py` on purpose. Everything in
`records` takes a `student_id` and filters on it because that is the scoping
guarantee; these functions deliberately return *many* students. Putting a scoped
and an unscoped query in one module invites one to be used where the other
belongs.

Filter dropdowns are queried rather than hardcoded, so they cannot offer a
programme that no longer exists — a small lie a UI tells confidently.

### Upload replaces a known document; it is not an upload endpoint

`POST /admin/documents/{filename}` accepts only the two filenames registered in
`ingestion/parse.py`. A PDF with no registered chunker would either be rejected
downstream or, worse, be run through a generic chunker — which is exactly what
Phase 3 argued against, and would put ungrounded text behind a citation.

Uploads land in a named `uploads` volume, and ingestion prefers a file there over
the shipped copy. `data/` stays bind-mounted read-only, so the frozen dataset is
never overwritten and deleting an upload reverts to the original rather than
leaving the corpus unrecoverable.

### Vite dev server in Docker, not a production build

The `frontend` service runs `npm run dev`, matching the backend's
`uvicorn --reload`. This is a development stack; HMR is worth more here than a
minified bundle, and a production image would be a second build stage serving
`dist/`. Both containers use the same anonymous-volume trick to stop the
Windows-native `node_modules` / `.venv` shadowing the Linux one.

CORS lists explicit origins rather than `*`, because these requests carry an
`Authorization` header and a wildcard origin with credentials is both refused by
browsers and the wrong thing to want.

### Three runtime dependencies

`react`, `react-dom`, `react-router-dom`. No UI framework, no state library, no
data-fetching library — the app is two surfaces over a REST API, `useState` and
`useEffect` carry it, and each of those would be scaffolding without a load.
Consistent with how the backend's dependencies were justified one at a time.

### Still open after Phase 6

- **Part 2 of this exit check has not been run**, along with Phases 3, 4 and 5's
  — all four wait on `OPENAI_API_KEY`. Part 1 (`--structural`) needs only the
  database, and that has not run either, because Docker has not been up.
- **The frontend has never been rendered.** It typechecks and builds
  (`tsc -b && vite build` clean), but no browser has loaded it, so layout and the
  API round-trips are unverified.
- **No admin audit log.** Settings changes are not recorded; the panel is a
  single shared login, so there is nobody to attribute them to anyway.
- **The browsers page in the API but not in the UI.** `offset`/`limit` exist and
  the total is honest, but the panel shows the first page only. At 5 students and
  84 enrollments nothing is truncated; at a real size this needs controls.
- **Admin sessions are not listed or revocable** beyond expiry.

---

## First run against Postgres — what actually broke

Docker Desktop came up for the first time after all seven phases were written.
Everything below had been reasoned about carefully and never executed, so this
section is the honest record of what that was worth.

### The bug: an anonymous volume served a venv from the first-ever build

The backend crashed on start with `ModuleNotFoundError: No module named 'pypdf'`
— a dependency that is in `pyproject.toml`, in `uv.lock`, and demonstrably in
the image.

The cause was Compose's `- /app/.venv` anonymous volume. It existed to stop the
`./backend:/app` bind mount hiding the container's Linux venv, and it did that
correctly. What it also did was **persist across `up --build`**: the venv created
on day one, before `pypdf`, `openai` and `python-multipart` were ever added, kept
being mounted over every later image. Three phases of dependencies were invisible.

The tempting fix is `--renew-anon-volumes` in the run instructions, which turns a
structural problem into something every future developer must remember. The fix
taken instead removes the shadowing entirely: `UV_PROJECT_ENVIRONMENT=/opt/venv`,
outside `/app`, so the bind mount cannot reach it and no anonymous volume is
needed. One line in the Dockerfile, one deletion from Compose.

The frontend keeps its `node_modules` anonymous volume — npm requires the
directory to sit inside the project — so it retains the same latent staleness
after a dependency change. Noted rather than solved.

### What running it proved, and what it caught

Everything that had been reasoned about held up:

- **All seven migrations applied cleanly**, and `alembic revision --autogenerate`
  against the upgraded database produced an **empty** revision. Revisions
  `0004`–`0007` were hand-written with no database to check against, so this was
  the real test of four migrations at once, and there is zero drift from the
  models.
- Phases 1 and 2 passed unchanged against real Postgres.
- The admin surface passed on the first run, including both directions of the
  cross-principal refusal.
- Jad's trap reproduces exactly as hand-computed in Phase 1: 55/72 credits, three
  categories unsatisfied, capstone at 0.

What it caught was **fixture assumptions, not logic**. Two Phase 5 expectations
were guesses about the five students that the data contradicted — see the
correction box in the Phase 5 section. The eligibility code was right both times;
the test and the design note were wrong.

That is the lesson worth keeping: the parts that were verified by construction
(schemas, gates, migrations, SQL against an independent implementation) survived
contact with reality. The parts asserted from memory about the dataset did not.
